...
async function testFolderProcessing() {
    try {
        const response = await axios.post('http://localhost:8081/api/event/processFolderAudio', {
...
async function testDefaultVoiceFolder() {
    try {
        const response = await axios.post('http://localhost:8081/api/event/processVoiceFolder', {}, {
            auth: {
                username: 'tipax',
                password: 'opera-qc-2024'
            },
