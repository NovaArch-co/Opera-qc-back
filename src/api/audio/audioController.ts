...
import { streamSessionEvents } from "./audioRepository";
...
class AudioController {
  ...
  public getSessionEvents: RequestHandler = async (req: Request, res: Response) => {
    try {
      ...
      const lastId = req.query.lastId ? Number.parseInt(req.query.lastId as string, 10) : undefined;
      const batchSize = req.query.batchSize ? Number.parseInt(req.query.batchSize as string, 10) : 1000;
      ...
      for await (const batch of streamSessionEvents(lastId, validBatchSize)) {
        if (!isFirstBatch) {
