...
export const sendFilesToTranscriptionAPI = async (filePathIn: string, filePathOut: string) => {
    try {
        ...
        const response = await axios.post("http://31.184.134.153:8000/transcribe/", form, {
            headers: {
                ...form.getHeaders(),
            },
        });
...
export const sendToAnalysisAPI = async (transcriptionData: any) => {
    try {
        ...
        const response = await axios.post("http://31.184.134.153:8000/analyze/", transcriptionData, {
            headers: {
