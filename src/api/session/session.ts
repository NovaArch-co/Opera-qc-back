...
export class SessionEventController {
    public createSessionEvent = async (req: Request, res: Response) => {
        try {
            ...
            console.log(
                `[API_CALL_DETAILS] Type: ${type}, Filename: ${filename}, Date: ${date}, Source: ${source_number}, Dest: ${dest_number}`,
            );
    ...
    public getSessionEventById: RequestHandler = async (req: Request, res: Response) => {
        ...
        try {
            const sessionEvent = await prismaClient.sessionEvent.findUnique({
                where: { id: Number(id) },
            });

            if (!sessionEvent) {
                return handleServiceResponse(
                    ServiceResponse.failure("Session event not found", {}, StatusCodes.NOT_FOUND),
                    res,
                );
            }

            const incommingfileUrl = sessionEvent.incommingfileUrl
                ? `${getMinioEndpoint()}${sessionEvent.incommingfileUrl}`
                : null;
            const outgoingfileUrl = sessionEvent.outgoingfileUrl
                ? `${getMinioEndpoint()}${sessionEvent.outgoingfileUrl}`
                : null;
            ...
            const sessionEvents = {
                ...
                topic:
                    sessionEvent.topic && typeof sessionEvent.topic === "object" && Object.keys(sessionEvent.topic).length > 0
                        ? Object.keys(sessionEvent.topic)[0]
                        : "",
                subTopic:
                    sessionEvent.topic && typeof sessionEvent.topic === "object" && Object.values(sessionEvent.topic).length > 0
            ...
            console.log(error);
            return handleServiceResponse(
                ServiceResponse.failure("Error fetching session event", error, StatusCodes.INTERNAL_SERVER_ERROR),
                res,
            );
        }
    };
    ...
    public getSessions: RequestHandler = async (req: Request, res: Response) => {
        try {
            ...
            console.log("Pagination and filter params:", {
                page,
                limit,
                offset,
                emotion,
            ...
            const totalCountResult = await prismaClient.$queryRawUnsafe<{ total: number }[]>(totalCountQuery, ...params);
            ...
            const sessionEvents = await prismaClient.$queryRawUnsafe(dataQuery, ...params, limit, offset);
            ...
            const formattedSessions = (sessionEvents as any[]).map((event) => ({
                ...
                topic:
                    event.topic && typeof event.topic === "object" && Object.keys(event.topic).length > 0
                        ? Object.keys(event.topic)[0]
                        : "",
                subTopic:
                    event.topic && typeof event.topic === "object" && Object.values(event.topic).length > 0
            ...
            if (formattedSessions.length === 0) {
                return handleServiceResponse(
                    ServiceResponse.success("No processed session events found", {
                        data: [],
                        pagination: {
                            currentPage: 1,
                            totalPages: 0,
            ...
            return handleServiceResponse(
                ServiceResponse.success("Session events retrieved successfully", {
    ...
    public getSessionStats: RequestHandler = async (req: Request, res: Response) => {
        try {
            ...
            const safeEmotionStats = this.convertBigIntToNumber(emotionStats[0]) || {
                top_emotion: null,
                top_emotion_count: 0,
            };
    ...
    public getAudioFile: RequestHandler = async (req: Request, res: Response) => {
        try {
            ...
            if (!filename || filename.includes("..") || filename.includes("/") || filename.includes("\\")) {
                return handleServiceResponse(ServiceResponse.failure("Invalid filename", {}, StatusCodes.BAD_REQUEST), res);
            }
        ...
        try {
            ...
            if (dirExists) {
                try {
                    ...
                    dirPermissions = `0${(dirStats.mode & 0o777).toString(8)}`;
            ...
            if (fileExists) {
                try {
                    ...
                    filePermissions = `0${(fileStats.mode & 0o777).toString(8)}`;
...
export const sendToAnalysisAPI = async (transcriptionData: any) => {
    try {
        ...
        const response = await axios.post("http://31.184.134.153:8003/analyze/", transcriptionData, {
            headers: {
                accept: "application/json",
