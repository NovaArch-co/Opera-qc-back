...
export const SessionEventSchema = z.object({
  ...
  type: z
    ...
    .openapi({ example: "incoming", description: "Call type - only incoming calls will be processed" }),
...
export const CreateSessionEventResponseSchema = z.object({
  ...
  incommingfileUrl: z
    .string()
    .nullable()
    .openapi({ example: "http://localhost:9000/audio-files/14030721-191913-09151532004-204-in.wav" }),
  outgoingfileUrl: z
    .string()
    .nullable()
    .openapi({ example: "http://localhost:9000/audio-files/14030721-191913-09151532004-204-out.wav" }),
  msg: z.string().openapi({ example: "Session event info" }),
  ...
  topic: z
    .array(z.string())
    .nullable()
    .openapi({ example: ["101"] }), // Updated to match new format
  emotion: z.string().nullable().openapi({ example: "ناراحت" }),
  keyWords: z
    .array(z.string())
    .nullable()
    .openapi({ example: ["شماره", "وارد", "مشتری", "برنامه", "دیدن"] }),
  forbiddenWords: z
    .record(z.string(), z.number())
    .nullable()
    .openapi({ example: { آهان: 2, آره: 1 } }),
