...
|---------|------------|------|-------------|
| **API Server** | 8081 | **8081** | Main application |
...
- **Voice Folder Processing:** `POST http://localhost:8081/api/event/processVoiceFolder`
...
```bash
curl -X POST http://localhost:8081/api/event/processVoiceFolder \
  -u tipax:opera-qc-2024 \
  -H 'Content-Type: application/json'
```
...
### Check Sessions:
```bash
curl -X GET http://localhost:8081/api/audio/sessions \
  -u tipax:opera-qc-2024
```

### Process Custom Folder:
```bash
curl -X POST http://localhost:8081/api/event/processFolderAudio \
  -u tipax:opera-qc-2024 \
  -H 'Content-Type: application/json' \
...
- Frontend URL (CORS_ORIGIN defaults to http://localhost:8082)
